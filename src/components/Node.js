...
class Node extends React.Component {
    constructor(props) {
        super(props);
        let {waves, valign, align, hideOn, float, truncate=false, hoverable=false, node='div', className, textColor, color, depth, ...remainingProps} = props;
    ...
    textColorPropToClassName(textColor) {
        this.className = cn(this.className, Utils.prefix(function(prop) {
    ...
    render() {
        let N = this.node;
        let component = <N ref='el' className={ this.className } {...this._props}>
                            { this._props.children }
                        </N>;
        if (this.valign && ['top', 'middle', 'bottom'].indexOf(this.valign)) {
            switch (this.valign) {
                case 'top':
                    return <div className='valign-wrapper' style={ { '-webkit-align-items': 'flex-start', '-ms-flex-align': 'flex-start', 'align-items': 'flex-start' } }>
                               { component }
                           </div>;
                case 'middle':
                ...
                case 'bottom':
                    return <div className='valign-wrapper' style={ { '-webkit-align-items': 'flex-end', '-ms-flex-align': 'flex-end', 'align-items': 'flex-end' } }>
                               { component }
                           </div>;
            }
        }

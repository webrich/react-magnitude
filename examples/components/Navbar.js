...
import { SideNav, Heading, Divider, Navs, Nav, Anchor, Collapsible } from "react-magnitude";
                ...
                <Collapsible>
                    {navigations.map(navigation => {
                        if (navigation.components) {
                            let components = navigation.components.sort((a, b) => {
                                return a.title > b.title;
                            });
                            let isActive = components.find(x => x.path === pathname) !== undefined;
                            return (
                                <Collapsible.Item key={navigation.title}>
                                    ...
                                    <Collapsible.Body>
                                        <Navs>
                                            {components.map(component => {
                                                if (component.title) {
                                                    return (
                                                        <Nav key={component.path} target={component.target} to={component.path}>
                                                            {component.title}
                                                        </Nav>
                                                    );
                                                }
